...
public class VerticalTabLayout extends NestedScrollView {
    ...
    public VerticalTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mIndicatorWidth = (int) typedArray.getDimension(R.styleable.VerticalTabLayout_indicator_width1, DisplayUtil.dp2px(context, 3));
        ...
        if (mIndicatorGravity == 80) {
            mIndicatorGravity = Gravity.BOTTOM;
        } else if (mIndicatorGravity == 3) {
    ...
    private class TabStrip extends LinearLayout {
        ...
        protected void setIndicatorGravity() {
            if (mIndicatorGravity == Gravity.LEFT) {
                ...
                mIndicatorX = 0;
                setPadding(0, 0, 0, 0);
            } else if (mColorIndicator == Gravity.BOTTOM) {
                mIndicatorX = 0;
                setPadding(0, 0, 0, mIndicatorWidth);
            }
            post(new Runnable() {
                @Override
                public void run() {
                    ...
                    } else if (mIndicatorGravity == Gravity.BOTTOM) {
                        mIndicatorX = 0;
                    }
                    invalidate();
                }
            ...
            post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (direction > 0) {
                        ...
                        endAnime = ValueAnimator.ofFloat(mIndicatorTopY, targetTop).setDuration(0);
                        ...
                        startAnime = ValueAnimator.ofFloat(mIndicatorTopY, targetTop).setDuration(0);
                        startAnime.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                            @Override
                            public void onAnimationUpdate(ValueAnimator animation) {
                                mIndicatorTopY = Float.parseFloat(animation.getAnimatedValue().toString());
                        ...
                        endAnime = ValueAnimator.ofFloat(mIndicatorBottomY, targetBottom).setDuration(0);
                        endAnime.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                            @Override
                            public void onAnimationUpdate(ValueAnimator animation) {
                                mIndicatorBottomY = Float.parseFloat(animation.getAnimatedValue().toString());
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (mIndicatorGravity == Gravity.BOTTOM) {
                mIndicatorRect.left = getRight() / 2 - 30;
                mIndicatorRect.top = mIndicatorBottomY - mIndicatorWidth;
                mIndicatorRect.right = getRight() / 2 + 30;
